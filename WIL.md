...
## [1 - UML](#1UML)
## [2 - 예외](#2예외)
## [3 - API와 BindingReuslt](#3API와-BindingReuslt)
...
예측 가능한 예외 - 개발자와 사용자 모두가 예쌍할 수 있는 오류
...
- 사용자에게는 "예상치 못한 오류" 메시지만 반환
  - ```java
    ...
    ```
### 2.4 결과
  프로젝트 내에서 예상이 가능한 부분에 대해서는 커스텀 예외를 만들어 활용하였다. 그 외에 예측 불가능한 예외는 Exception으로 묶어 응답하는 설계를 하였다.
### 2.5 소감
  개발자는 사용자의 사용경험을 저해하지 않도록 예외에 대하여 촘촘히 설계 하되, 예측 불가능한 부분에 대해서는 기록하고 사후를 빠르게 해야겠다고 느겼다.


## 3.API와 BindingReuslt
### 3.1 목적
